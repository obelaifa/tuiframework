...
public class PortSet {
	...
	// private Hashtable<String, PortEntry> portEntryMap = new Hashtable<String,
	// PortEntry>();
	...
	public void add(Port port) {
		if ((port.getDataFlowDirection() & Port.Source) != 0) {
			this.sources.add(port.getName());
		}

		if ((port.getDataFlowDirection() & Port.Sink) != 0) {
	...
	/*
	 * public Hashtable<String, PortEntry> getPortEntryMap() { return
	 * portEntryMap; }
	 */
	...
	public Dimension minSize(Graphics2D g2d) {
		...
		int mw1 = 0;
		{
			...
			while (iter.hasNext()) {
				...
				mw1 = Math.max(mw1, f.stringWidth(iter.next()) + portCircleBorder.getLeft() + portCircleSize
						+ portCircleBorder.getRight());
			}
		}
		...
		int mw2 = 0;
		{
			...
			while (iter.hasNext()) {
				...
				mw2 = Math.max(mw2, f.stringWidth(iter.next()) + portCircleBorder.getLeft() + portCircleSize
						+ portCircleBorder.getRight());
			}
		}
	...
	public void calculateOffsets(Graphics2D g2d, int width, int heightOffset) {
		...
		int h1;
		{
			...
			while (iter.hasNext()) {
				...
				String portIdName = "Sink_" + portName;

				this.portEntryMap.put(portIdName, new PortEntry(
						portIdName,
						new Point(border.getLeft() + portCircleBorder.getLeft() + portCircleSize
								+ portCircleBorder.getRight(), h1 - lineLeading),
						new Point(border.getLeft() + portCircleBorder.getLeft(), h1 - portCircleSize - lineLeading),
						portName, h1, new Point(0, h1 - portCircleSize / 2 - lineLeading), false, this));
			}
		}

		{
			...
			while (iter.hasNext()) {
				...
				h1 += f.getAscent() + f.getDescent() + lineLeading;
				String portName = iter.next();
				String portIdName = "Source_" + portName;
				int w2 = f.stringWidth(portName);

				this.portEntryMap.put(portIdName, new PortEntry(
						portIdName,
						new Point(width - border.getRight() - w2 - portCircleBorder.getLeft() - portCircleSize
								- portCircleBorder.getRight(), h1 - lineLeading),
						new Point(width - border.getRight() - portCircleBorder.getRight() - portCircleSize,
								h1 - portCircleSize - lineLeading),
						portName, h1, new Point(width, h1 - portCircleSize / 2 - lineLeading), true, this));
			}
		}
	}
	...
	public void draw(Graphics2D g2d, Point offset, int width) {
		...
		while (iter.hasNext()) {
			...
			g2d.fillOval(offset.x + p.getCircleOffset().x, offset.y + p.getCircleOffset().y, portCircleSize,
					portCircleSize);
			g2d.setColor(new Color(64, 64, 64));
			g2d.drawOval(offset.x + p.getCircleOffset().x, offset.y + p.getCircleOffset().y, portCircleSize,
					portCircleSize);
			...
			/*
			 * g2d.setColor(new Color(255, 0, 0)); g2d.drawLine(offset.x +
			 * p.getConnectionPoint().x - 1, offset.y +
			 * p.getConnectionPoint().y, offset.x + p.getConnectionPoint().x +
			 * 1, offset.y + p.getConnectionPoint().y); g2d.drawLine(offset.x +
			 * p.getConnectionPoint().x, offset.y + p.getConnectionPoint().y +
			 * 1, offset.x + p.getConnectionPoint().x, offset.y +
			 * p.getConnectionPoint().y - 1);
			 */
		}
	}

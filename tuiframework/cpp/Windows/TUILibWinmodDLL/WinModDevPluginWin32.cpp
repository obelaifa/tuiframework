...
void dllInitPlugin() {
	DeviceFactorySingleton::getInstance()->registerCreateFunction(WinModDev::getDeviceName(), WinModDev::createFunction);
    deviceTypeNameVector.push_back(WinModDev::getDeviceName());
}
...
void dllDeviceRegistration(IDeviceFactory * deviceFactory) {
    deviceFactory->registerCreateFunction(WinModDev::getDeviceName(), WinModDev::createFunction);
}
